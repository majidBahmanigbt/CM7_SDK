...
bool is_remote_addr_initialized = false;
...
void send_task(void *param){
    ...
    {
        printf("send\r\n");
        if(is_remote_addr_initialized){

            tx_buf = rpmsg_lite_alloc_tx_buffer(my_rpmsg, &size, RL_BLOCK);
            assert(tx_buf);
            /* Copy string to RPMsg tx buffer */
            memcpy(tx_buf, "EFACEC", len);
...
void app_task(void *param)
{
    ...
    for (;;)
    {
        ...
        if ((len == 2) && (app_buf[0] == 0xd) && (app_buf[1] == 0xa)){
            ...
            is_remote_addr_initialized = !is_remote_addr_initialized;
...
void app_create_task(void)
{
    ...
    xTaskCreate(send_task, "SEND_TASK", APP_TASK_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL);
